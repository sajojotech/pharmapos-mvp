...
export async function GET(request: Request) {
  ...
  const csv = toCsv(
    ["Tanggal", "Cabang", "Jumlah Transaksi", "Omzet"],
    rows.map((r) => [r.date, `${r.branchCode} — ${r.branchName}`, r.transactionCount, r.grossSales.toString()]),
  );
  return csvResponse(`penjualan-per-cabang_${dateFrom}_${dateTo}.csv`, csv);
}
