...
export default async function StockByBatchReportPage({
  searchParams,
}: {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Stok per Batch</h1>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        />
        <ReportExportLink
          basePath="/reports/stock-by-batch"
          filters={{ branchId, productId, status, expiryFrom, expiryTo }}
        />
      </div>
          ...
          <tbody>
            ...
            {data.map((batch) => (
              <tr key={batch.id} className="border-b border-slate-100">
                ...
                <td className="px-4 py-2 text-right">{formatRupiah(batch.unitCost.toString())}</td>
                <td className="px-4 py-2">
