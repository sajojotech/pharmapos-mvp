...
export async function GET(request: Request) {
  ...
  const windowDays = await resolveNearExpiryWindowDays(access.user.companyId);
  const { data } = await getNearExpiryReport({
  ...
  return csvResponse("produk-mendekati-ed.csv", csv);
}
